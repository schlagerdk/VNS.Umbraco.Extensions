  ...
  override render() {
    ...
    if (this.mode === 'tags') {
      ...
      return html`${this._items.map(
        (item) =>
          html`<uui-tag color=${this.color ?? 'default'} look=${this.look ?? 'secondary'} style=${`${badgeStyle};margin-right:6px;`}>${item}</uui-tag>`
      )}`;
    }
...
function toTagItems(value: unknown, separator?: string): string[] {
